...
final class ApiKeyService
{
    ...
    public function create(string $name, string $ownerUsername, array $roles): array {
        ...
        if ($owner === null || !$owner['enabled']) throw new ApiRequestException('API key owner is unavailable.', 'API_KEY_OWNER_UNAVAILABLE', [], 409);
    ...
    public function authenticate(string $raw): ?array {
        ...
        $owner=$this->users->findUserById($key['ownerUserId']); if ($owner===null || !$owner['enabled']) return null;
        $now=gmdate(DATE_ATOM); $this->keys->update(function (&$value) use ($key,$now) { foreach($value['keys'] as &$item) if(hash_equals($item['id'],$key['id'])){$item['lastUsedAt']=$now;break;} });
