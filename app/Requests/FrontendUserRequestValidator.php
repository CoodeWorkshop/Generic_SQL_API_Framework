...
final class FrontendUserRequestValidator
{
    ...
    public function validate(array $request):array
    {
        ...
        $allowed=match($action){'auth.frontendUsers.create'=>['action','username','password','passwordConfirmation','enabled','frontendRole'],'auth.frontendUsers.update'=>['action','username','newUsername'],'auth.frontendUsers.changePassword'=>['action','username','newPassword','passwordConfirmation'],'auth.frontendUsers.assignRole'=>['action','username','frontendRole'],default=>['action','username']};
        ...
        if($action==='auth.frontendUsers.create'){$this->password($request['password']??null,'password',$result,$details);$this->confirm($request['passwordConfirmation']??null,$result['password']??null,'passwordConfirmation',$details);$enabled=$request['enabled']??true;if(!is_bool($enabled))$details[]=['path'=>'enabled','message'=>'Enabled status must be boolean.'];else$result['enabled']=$enabled;$result['frontendRole']=$this->role($request['frontendRole']??null,$details);}
        elseif($action==='auth.frontendUsers.update'){try{$result['newUsername']=UsernamePolicy::normalize($request['newUsername']??null);}catch(InvalidArgumentException $e){$details[]=['path'=>'newUsername','message'=>$e->getMessage()];}}
        ...
        elseif($action==='auth.frontendUsers.assignRole')$result['frontendRole']=$this->role($request['frontendRole']??null,$details);
