...
final class ApiKeyRepository
{
    ...
    public function load(): array { RuntimeConfiguration::ensure(); $value = JsonFileStore::load($this->path); $this->validate($value); return $value; }
    public function update(callable $operation) {
        ...
        try { $value = JsonFileStore::load($this->path); $this->validate($value); $result = $operation($value); $this->validate($value); JsonFileStore::save($this->path, $value); return $result; }
    ...
    private function validate(array $value): void {
        if (($value['version'] ?? null) !== 1 || array_diff(array_keys($value), ['version', 'keys']) !== [] || !is_array($value['keys'] ?? null) || !array_is_list($value['keys'])) throw new RuntimeException('Invalid API key configuration.');
        $ids = [];
        foreach ($value['keys'] as $key) {
            if (!is_array($key) || array_diff(array_keys($key), ['id','name','ownerUserId','roles','secretHash','fingerprint','enabled','revokedAt','createdAt','lastUsedAt']) !== []
                ...
                || preg_match('/^[a-f0-9]{32}$/', $key['ownerUserId'] ?? '') !== 1 || !is_array($key['roles'] ?? null) || !array_is_list($key['roles']) || $key['roles'] === []
