...
The supported Phase 4.2 production process models are IIS with PHP FastCGI on Windows and Nginx with PHP-FPM on Linux. Web-server/FastCGI workers own production concurrency and service lifecycle. When `GENERIC_APP_ENV=production`, Admin reports API and SQL Parser as externally managed, hides local lifecycle controls, and rejects their lifecycle actions; it never issues IIS, Nginx, PHP-FPM, systemd, or Windows service commands. Development retains the fixed local child-process managers. API, loopback Admin, and SQL Parser remain separate routing boundaries with fixed public entry points.
